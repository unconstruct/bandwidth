    ...
    if (!empty($object->list->story)) {
      foreach ($object->list->story as $story) {
        ...
        for($xml_iterator->rewind(); $xml_iterator->valid(); $xml_iterator->next()) {
          ...
          if ($key == 'image' || $key == 'audio' || $key == 'link' || $key == 'externalAsset' || $key == 'multimedia' || $key == 'staticGraphic' || $key == 'byline' || $key == 'parent' || $key == 'product') {
            // images
            if ($key == 'image') {
			...
			if ($key == 'parent'){
				$parsed->{'tax'}[] = $this->parse_simplexml_element($current); 	
			}
			if ($key == 'product'){
				$parsed->{$key}[] = $this->parse_simplexml_element($current);  				
			}
  ...
  function parse_simplexml_element($element) {
    ...
    if (count($element->children())) { // works for PHP5.2
      foreach ($element->children() as $i => $child) {
        if ($i == 'paragraph' || $i == 'mp3' || $i == 'link' || $i == 'purchaseLink' || $i == 'crop') {
          if ($i == 'paragraph') {
        ...
        if ($i == 'purchaseLink'){
	      	$pLink = $this->parse_simplexml_element($child);
            $NPRMLElement->productLink[$pLink->vendor] = $pLink;  
          }
        if ($i == 'crop'){
	        $crop = $this->parse_simplexml_element($child);
	        $NPRMLElement->crop[$crop->type] = $crop;
